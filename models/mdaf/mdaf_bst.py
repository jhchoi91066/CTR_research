...
from models.baseline.bst import BST
from models.mdaf.mdaf_components import GatedFusion, PredictionHead


class MDAF_BST(nn.Module):
    """
    MDAF with BST as sequential branch.

    This model integrates static feature modeling (DCNv3) with sequential
    behavioral modeling (BST) through an adaptive gated fusion mechanism.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            bst_embed_dim: BST embedding dimension
            bst_max_seq_len: Maximum sequence length
            bst_num_transformer_layers: Number of Transformer layers
            bst_num_heads: Number of attention heads
            bst_d_ff: Feed-forward dimension
            bst_dropout: BST dropout rate
        ...
        """
        super(MDAF_BST, self).__init__()

        # Critical: BST embed_dim must match embedding_dim for fusion
        assert bst_embed_dim == embedding_dim, \
            f"BST embed_dim ({bst_embed_dim}) must match embedding_dim ({embedding_dim})"
        ...
        self.bst = BST(
            ...
            other_feature_dims=other_feature_dims,
            embed_dim=bst_embed_dim,  # Must equal embedding_dim
            max_seq_len=bst_max_seq_len,
            num_transformer_layers=bst_num_transformer_layers,
            num_heads=bst_num_heads,
            d_ff=bst_d_ff,
            dropout=bst_dropout
        )
    ...
    def forward(
        ...
        target_category,
        item_history,
        category_history,  # Not used by BST but kept for compatibility
        other_features,
        return_gate=False
    ):
        """
        Args:
            ...
            category_history: (batch_size, seq_len) - category sequence (unused)
        ...
        # === Sequential Branch: BST ===
        # Note: BST uses item_history but not category_history
        sequential_emb = self.bst(
            target_item=target_item,
            target_category=target_category,
...
if __name__ == '__main__':
    """Test MDAF_BST with dummy Taobao data"""
    print("="*60)
    print("Testing MDAF_BST Model")
    print("="*60)
    ...
    model = MDAF_BST(
        ...
        dcnv3_embed_dim=16,
        bst_embed_dim=128,  # Must match embedding_dim for fusion
        embedding_dim=128
    )
