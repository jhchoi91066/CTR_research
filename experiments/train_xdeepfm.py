...
Usage:
    python experiments/train_xdeepfm.py --epochs 10 --batch_size 1024
"""
...
from models.baseline.xdeepfm import xDeepFM
...
def main():
    parser = argparse.ArgumentParser(description='xDeepFM 학습')
    parser.add_argument('--data_dir', type=str, default='data/processed')
    parser.add_argument('--batch_size', type=int, default=1024)
    parser.add_argument('--epochs', type=int, default=10)
    parser.add_argument('--lr', type=float, default=0.001)
    parser.add_argument('--embed_dim', type=int, default=16)
    parser.add_argument('--cin_layer_sizes', type=int, nargs='+', default=[128, 128])
    ...
    model = xDeepFM(
        ...
        cin_layer_sizes=args.cin_layer_sizes,
        cin_split_half=True,
    ...
    print(f"   - 범주형 특징: {len(metadata['cat_vocab'])}개")
    print(f"   - Total fields: {model.num_fields}")
    print(f"   - Embedding 차원: {args.embed_dim}")
    print(f"   - CIN 구조: {args.cin_layer_sizes}")
    print(f"   - DNN 구조: {args.dnn_hidden_units}")
