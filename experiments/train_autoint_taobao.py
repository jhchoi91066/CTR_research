"""
Train AutoInt model on Taobao dataset (static features only)

주의: AutoInt는 순차 모델링 능력이 없으므로,
...
from models.baseline.autoint import AutoInt
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"🎯 AutoInt on Taobao (Static Features)")
    print(f"{'='*60}")
    ...
    for epoch in range(args.epochs):
        ...
        if val_auc > best_auc:
            ...
            torch.save(model.state_dict(), 'results/checkpoints/autoint_taobao_best.pth')
            print(f"   ✅ Best Validation AUC: {best_auc:.4f}")

    print(f"\n{'='*60}")
    print(f"🎉 최종 Best Validation AUC: {best_auc:.4f}")
    print(f"{'='*60}")
    print(f"\n💾 모델 저장: results/checkpoints/autoint_taobao_best.pth")
